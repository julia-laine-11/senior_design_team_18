...
#
# Game mode OFF:
#   - Manual numpad / WASD / arrow / DualShock control
#
# Dependencies:
#   pip install opencv-python numpy pyserial pygame
# ==============================
import numpy as np
import time
import cv2
...
def tracking_thread(state, stop_event):
    ...
    if pygame is not None:
        ...
        if pygame.joystick.get_count() > 0:
            ...
            joystick.init()
            print(f"[Input] Wired Controller Ready: {joystick.get_name()}")
        else:
            print("[Input] No controller found. Defaulting to keyboard.")

    print("\nControls:")
    print("  G = Toggle game mode")
    print("  A = Toggle Auto-Clear mode")
    print("  Numpad / WASD / Arrows / DualShock = Manual drive (game OFF)")
...
def _inner_loop(state, stop_event, ctrl, cap,
    ...
    while not stop_event.is_set():
        ...
        # ---- DualShock Controller Handling ----
        if joystick and not game_on:
            pygame.event.pump()
            
            # Left Stick on DualShock: Axis 0 is Horizontal, Axis 1 is Vertical
            ax = joystick.get_axis(0)
            ay = joystick.get_axis(1)

            j_dx, j_dy = 0, 0
            deadzone = 0.5  # Large deadzone so we only read intentional pushes

            # X-Axis (Left is negative, Right is positive)
            if ax < -deadzone: j_dx = -1
            elif ax > deadzone: j_dx = 1

            # Y-Axis (Up is negative, Down is positive. Matches W/S inversion)
            if ay < -deadzone: j_dy = 1   # UP (Matches 'w')
            elif ay > deadzone: j_dy = -1 # DOWN (Matches 's')

            # Override keyboard dx/dy if the stick is being pushed
            if j_dx != 0 or j_dy != 0:
                dx, dy = j_dx, j_dy
                last_key_time = now_key # Reset the timeout to keep motors alive
