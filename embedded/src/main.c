...
// // Volatile variable to track milliseconds
// volatile uint32_t msTicks = 0;

// // The SysTick interrupt handler fires every time the timer hits zero
...
// int main(void) {
//     // Initialize the core clock variable (This syncs with your 48MHz f_cpu setting)
//     SystemCoreClockUpdate();
    
//     // Configure the internal SysTick timer to trigger an interrupt every 1 millisecond
//     SysTick_Config(SystemCoreClock / 1000);

//     // 1. ENABLE THE CLOCKS
//     // The GPIO ports are dead by default to save power. We must enable the 
//     // clock for Port B and Port C on the Advanced High-performance Bus (AHB).
//     RCC->AHBENR |= RCC_AHBENR_GPIOBEN | RCC_AHBENR_GPIOCEN;

//     // 2. CONFIGURE PC3 AS OUTPUT
//     // The MODER register controls pin mode. We clear the 2 bits for Pin 3, 
//     // then set them to '01' to designate it as a General Purpose Output.
//     GPIOC->MODER &= ~GPIO_MODER_MODER3;     // Clear bits 6 and 7
//     GPIOC->MODER |= GPIO_MODER_MODER3_0;    // Set bit 6

//     // 3. CONFIGURE PB15 AS OUTPUT
//     GPIOB->MODER &= ~GPIO_MODER_MODER15;    // Clear bits 30 and 31
//     GPIOB->MODER |= GPIO_MODER_MODER15_0;   // Set bit 30

//     // Main control loop
//     while (1) {
//         // Toggle the pins using the Output Data Register (ODR)
//         GPIOC->ODR ^= (1 << 3);
//         GPIOB->ODR ^= (1 << 15);
