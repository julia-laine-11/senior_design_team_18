...
//     GPIOC->BRR  = (1 << 9);        // EN+ LOW  (Driver Enabled)
...
//             // Extract the direction flag (MSB: 0x80 is 1000 0000 in binary)
//             uint8_t is_reverse = (rx_byte & 0x80); 
            
//             // Extract the frequency magnitude (Lower 7 bits: 0x7F is 0111 1111)
//             // Add the 100 base offset back to it
//             uint32_t decoded_freq = (rx_byte & 0x7F) + 100;
            
//             // Apply the updates if the frequency is within valid bounds
